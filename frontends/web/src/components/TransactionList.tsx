    ...
    return (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden flex flex-col h-[600px]">
            <div className="p-6 border-b border-slate-800 flex justify-between items-center">
                <h3 className="text-slate-100 text-lg font-semibold">Transactions</h3>
                    ...
                    <tbody className="divide-y divide-slate-800">
                        {sorted.map(t => (
                            <tr key={t.id} className="hover:bg-slate-800/50 transition-colors">
                                <td className="p-4 text-sm text-slate-400 whitespace-nowrap">{format(t.date, 'MMM dd, yyyy')}</td>
                                <td className="p-4 text-sm text-slate-200 font-medium">{t.description}</td>
                                <td className="p-4 text-sm">
                                    <span className="bg-slate-800 text-slate-400 px-2 py-1 rounded-md text-xs border border-slate-700">
                                        {t.category}
                                    </span>
                                </td>
                                <td className={`p-4 text-sm font-semibold text-right ${t.isIncome ? 'text-emerald-400' : 'text-slate-200'}`}>
